...
const { end_cursor } = useCache().value;
...
function finishExport() {
    ...
    // 添加计数器，活跃后求赞功能
    const { active_count, is_recommend } = useCache().value;
    active_count++;
    let RECOMMEND_COUNT = 3;

    if (active_count % RECOMMEND_COUNT == 0 && !is_recommend) {
        Dialog.confirm({
            title: 'Invitation',
            message: "Did the plugin help you? We need your encouragement, give us a 5-star review!",
